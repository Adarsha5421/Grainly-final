...
const createBooking = async (req, res) => {
  try {
    ...
    const { pulses, quantity, totalPrice, address, phone, weight, pricePerGram, paymentMethod, khaltiTransactionId } = req.body;
...
const getUserBookings = async (req, res) => {
  try {
    ...
    const bookings = await Booking.find({ user: userId }).populate("pulses");
    res.json(bookings);
  } catch (err) {
    res.status(500).json({ msg: err.message });
  }
...
const getAllBookings = async (req, res) => {
  try {
    ...
    const bookings = await Booking.find().populate("user pulses");
    res.json(bookings);
  } catch (err) {
    res.status(500).json({ msg: err.message });
  }
